                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                <input type="text" name="name" class="form-control" placeholder=""
                                       v-validate="'required'" data-vv-as="用户名称"/>
...
export default {
  ...
  mounted () {
    ...
    setTimeout(function () {
      db.getRequest('Institution/User/getNumber', {}).then(res => {
        if (res.status === 1) {
          self.number = res.data.user_number
    ...
    validateBeforeSubmit () {
      ...
      self.$validator.validateAll().then((result) => {
        if (result) {
          ...
          db.postRequest('Institution/User/add', params).then(res => {
            if (res.status === 1) {
