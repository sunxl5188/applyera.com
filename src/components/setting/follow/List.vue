<template>
    <div>
        <div :style="name==='follow'?'display: block;':'display: none;'">
            <div class="clearfix pb-30">
                <div class="row">
                    ...
                    <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6 form-inline text-right">
                        ...
                        <div class="form-group ml-10">
                            <div class="dropdown">
                                <router-link to="/setting/follow/detail" class="btn btn-default"><i class="iconfont">&#xe73e;</i>
                                    添加
                                </router-link>
                            </div>
                        </div>
            ...
            <div class="row">
                <table class="table table-customize">
                    ...
                    <tr v-for="(item, i) in list" :key="i">
                        ...
                        <td><span class="label is-round" :style="'background-color:'+item.status_color+';'">{{item.follow_type}}</span></td>
                        ...
                        <td class="text-center">
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li>
                                        <router-link
                                                :to="{path:'/setting/follow/detail',query:{id:item.id}}">编辑
                                        </router-link>
                                    </li>
                                    <li>
...
export default {
  ...
  methods: {
    pagechange (p) {
      ...
      db.postRequest('Institution/FollowType/index', params).then(res => {
        if (res.status === 1) {
    ...
    setDisable (id, i) {
      ...
      db.postRequest('Institution/FollowType/changeStatus', params).then(res => {
        if (res.status === 1) {
          self.list.map(item => {
            if (item.id === id) {
              item.status = i
